...
function conectados(socket) {
    ...
    if (Object.keys(Poscicion_Jugadores).length === 0){
       console.log('Jugador 1 conectado ') ;
       serverBalls[socket.id] = new Ball(80, 270, /*150, 270*/ 25, 10);
        serverBalls[socket.id].maxSpeed = 4;
        serverBalls[socket.id].score = 0
        serverBalls[socket.id].no = 1;
        Poscicion_Jugadores[socket.id] = {x: 115, y: 270, no: 1};
    }
    else if (Object.keys(Poscicion_Jugadores).length === 1){
        console.log("jugador 2 conectado");
        //creating player 2
       serverBalls[socket.id] = new Ball(560, 270, /*490, 270,*/ 25, 10);
        serverBalls[socket.id].maxSpeed = 5;
        serverBalls[socket.id].score = 0;
        serverBalls[socket.id].no = 2;
        Poscicion_Jugadores[socket.id] = {x: 525, y: 270, no: 2};
        pelota_hockey = new Ball(320, 270, 20, 6);
        io.emit('actualizar_posicion_pelota', {x: pelota_hockey.pos.x, y: pelota_hockey.pos.y});
    }
    else if (Object.keys(Poscicion_Jugadores).length > 1){
        console.log("existen demasiados jugadores para empezar la partida");
        return false;
    }

    io.emit('actualizar_jugadores', Poscicion_Jugadores);
    socket.on('disconnect', function () {
        serverBalls[socket.id].remove();
        delete serverBalls[socket.id];
        delete Poscicion_Jugadores[socket.id];
        console.log('jugador ' + socket.id + ' desconectado');
        console.log('numero de jugadres conectados: ' + Object.keys(Poscicion_Jugadores).length)
        io.emit('actualizar_jugadores', Poscicion_Jugadores);
    });
...
function serverLoop(){
    ...
    for (let id in serverBalls){
        ...
        Poscicion_Jugadores[id].x = serverBalls[id].pos.x;
        Poscicion_Jugadores[id].y = serverBalls[id].pos.y;
        Poscicion_Jugadores[id].angle = serverBalls[id].angle;
    }
    ...
    if (pelota_hockey === undefined){
        ...
        gameLogic();
        posicionPelota = {x: pelota_hockey.pos.x, y: pelota_hockey.pos.y};
    }
    //console.log('emit')
    io.emit('actualizar_Posicion', Poscicion_Jugadores);
    io.emit('actualizar_posicion_pelota', posicionPelota);
}
...
function gameSetup(){
    ...
    pelota_hockey.pos.set(320, 270);
    pelota_hockey.vel.set(0, 0);
}
...
function gameOver(){
    ...
    gameSetup();
    io.emit('actualizar_Marcador', null);
    setTimeout(() => {
        for(let id in serverBalls){
...
function gameLogic(){
    if(pelota_hockey.pos.x < 45 || pelota_hockey.pos.x > 595){
        scoring();
    }
    for(let id in serverBalls){
...
function scoring() {
    let marcador_Usuario;
    if (pelota_hockey.pos.x < 45) {
        for (let id in serverBalls) {
            if (serverBalls[id].no === 2) {
    ...
    gameSetup();
    io.emit('actualizar_Marcador', marcador_Usuario);
